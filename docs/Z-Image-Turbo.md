...
输入token数量，prompt_extend=false时固定为0。

output_tokens integer

输出token数量，prompt_extend=false时固定为0。

output_tokens_details object

输出 token 详情，仅当prompt_extend=true时返回。
...
总token数量，prompt_extend=false时固定为0。
